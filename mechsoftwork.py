...
def fix_text(mystr):
    wordArr = mystr.split(" ")
    for index, i in enumerate(wordArr):
        if bool(re.findall(r'(\(\w+\))', i)) == True:
            wordArr[index] = re.search(r'(\(\w+\))', i).string.replace("(", "", 1).replace(")", "", 1)
        else:
            wordArr[index] = (''.join(list(reversed(i))))
    return " ".join(wordArr)
